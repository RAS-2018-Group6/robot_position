#ifndef movement_class
#define movement_class
...
public:
  ...
  void executeCB(const actionlib_movement::MovementGoalConstPtr &goal)
  {
    ...
    PathCreator my_path;
    std::vector<double> path_points = my_path.getPath(x0,y0,x1,y1);; // JONATHAN! Assume points are stored like this (x0,y0,x1,y1,x2,y2....)
    ...
    while (i<=mypath.getNumber())
    {
      ...
      ROS_INFO("movement_class: Looping through all points.");
			while (sqrt(pow(path_points[i][0]-x_robot,2)+pow(path_points[i][1]-y_robot,2))<= radius)
      {
        i++;
      }
